...
def init_default_admin():
    """初始化默认管理员密码"""
    if DEFAULT_ADMIN['password_hash'] is None:
        # 默认密码是 'admin123'，建议在生产环境中修改
        default_password = 'admin123'
        DEFAULT_ADMIN['password_hash'] = hash_password(default_password)
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not admin_username or admin_username.lower() != DEFAULT_ADMIN['username'].lower():
            flash('权限不足', 'danger')
            admin_logout()
            return redirect(url_for('admin.login'))
...
def change_admin_password(old_password, new_password):
    """修改管理员密码"""
    # 验证旧密码
    if not verify_password(DEFAULT_ADMIN['password_hash'], old_password):
        return False, '当前密码错误'
...
def get_admin_info():
    ...
    return {
        ...
        'email': DEFAULT_ADMIN['email'],
        'has_password': DEFAULT_ADMIN['password_hash'] is not None
    }
